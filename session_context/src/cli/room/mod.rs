...
pub async fn handle_room_command(
  command: RoomCommand, user_service: UserService, room_service: RoomService, session_service: crate::services::SessionService,
  ...
  match command {
    RoomCommand::List {
      ...
      offset,
      limit,
    } => list::execute(session_service, room_service, session_id, offset, limit).await,
    RoomCommand::Create {
      session_id,
      name,
      max_players,
