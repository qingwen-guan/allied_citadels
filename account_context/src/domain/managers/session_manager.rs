...
impl SessionManager {
  ...
  pub async fn create_session(&self, account_id: AccountId) -> Result<SessionId, AccountError> {
    let session_id = SessionId::make();
    let expires_at = chrono::Utc::now() + chrono::Duration::hours(self.session_duration_hours as i64);
    self
      .session_repository
      .create(session_id, account_id, expires_at)
      .await?;
    Ok(session_id)
  }
  ...
  pub async fn delete_session(&self, session_id: SessionId) -> Result<bool, AccountError> {
    self.session_repository.delete(session_id).await
  }

  /// Delete all expired sessions
  pub async fn delete_expired_sessions(&self) -> Result<u64, AccountError> {
    self.session_repository.delete_expired().await
  }

  /// Delete all sessions for a specific account
  pub async fn delete_sessions_by_account(&self, account_id: AccountId) -> Result<u64, AccountError> {
    self.session_repository.delete_by_account_id(account_id).await
  }

  /// Set expiration time for all active sessions of a specific account
  /// This gives existing sessions a grace period before they expire
  /// Returns the number of sessions that were updated
  pub async fn set_expiration_for_account_sessions(
    &self, account_id: AccountId, expires_at: chrono::DateTime<chrono::Utc>,
  ) -> Result<u64, AccountError> {
    self
      .session_repository
      .update_expiration_by_account_id(account_id, expires_at)
      .await
  }
  ...
  pub async fn get_session(&self, session_id: SessionId) -> Result<Option<SessionInfo>, AccountError> {
    self.session_repository.get_by_session_id(session_id).await
  }
}
